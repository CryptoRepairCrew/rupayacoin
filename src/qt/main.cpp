...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RupayaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RupayaCoin Signed Message:\n";
...
{
    // RupayaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    // i think this will fix the issue of RupayaCoin total value of coins.
...
static const int64 nTargetTimespan = 4 * 60 * 60; // RupayaCoin: 4 hours
static const int64 nTargetSpacing = 1 * 60; // RupayaCoin: 1 Minute Blocks
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
   ...
   unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
    ...
    // RupayaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // RupayaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
/*
2014-02-05 01:15:07 block.nTime = 1388880557 
2014-02-05 01:15:07 block.nNonce = 387006691 
2014-02-05 01:15:07 block.GetHash = b78197f0e175697646db1f738edc1ffdcb30588ebe70e7e16026489076577061
*/
        // Genesis block
...
unsigned char pchMessageStart[4] = { 0xaa, 0xaa, 0xaa, 0xcc };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RupayaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RupayaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RupayaCoinMiner(CWallet *pwallet)
{
    printf("RupayaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("RupayaCoin-miner");
    ...
    try { loop {
        ...
        printf("Running RupayaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RupayaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RupayaCoinMiner, pwallet));
}
