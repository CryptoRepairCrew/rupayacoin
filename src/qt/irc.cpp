...
// Copyright (c) 2011-2012 RupayaCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("RupayaCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        CService addrConnect("92.51.132.40", 6667); // irc.leafco.in
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #RupayaCoinTEST0\r");
            Send(hSocket, "WHO #RupayaCoinTEST0\r");
        } else {
            // randomly join #RupayaCoin00-#RupayaCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #RupayaCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #RupayaCoin%02d\r", channel_number).c_str());
        }
